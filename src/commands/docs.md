...
- Commands cover the full newsletter workflow: low-level SES operations (contacts, send, bulk-send, suppression, lists, stats, templates, identities, config-sets, validate, jobs) and high-level abstraction commands (health, preflight, cleanup, setup) that compose multiple service calls into newsletter-specific workflows
