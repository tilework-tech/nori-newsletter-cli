...
- Commands cover the full newsletter workflow: low-level SES operations (contacts, send, bulk-send, suppression, lists, stats, templates, identities, config-sets, validate, jobs) and high-level abstraction commands (health, preflight, cleanup, setup, reputation, domain-check, audit) that compose multiple service calls and/or external lookups into newsletter-specific workflows
