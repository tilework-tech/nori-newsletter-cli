...
- Commands cover the full newsletter workflow: low-level SES operations (contacts, send, bulk-send, suppression, lists, stats, templates, identities, config-sets, validate, jobs) and high-level abstraction commands (health, preflight, cleanup, setup, reputation, domain-check, audit, send-safe) that compose multiple service calls and/or external lookups into newsletter-specific workflows
...
- **Open/click tracking (`send`, `send-safe`, `bulk-send`):** all three send commands attach the optional `configurationSetName` from `newsletter.config.json` plus per-issue SES message tags `campaign` and `source=newsletter`, built via `@/src/lib/campaign.ts`. `send`/`send-safe` derive the campaign id from the HTML filename (`campaignIdFromFile`); `bulk-send` has no HTML file so it sanitizes the template name. A configuration set is what makes SES emit open/click events, so when it is absent the commands send **no** configuration set and **no** tags (tags without a config set are meaningless) and instead print one `Warning: no configurationSetName ...` line to stderr per run -- once per run, not per recipient or batch, and never a hard error. `--dry-run` prints the campaign id (`[dry run] Campaign: <id>`) so it can be inspected before sending
...
- **`domain-check` command** performs comprehensive DNS and SES identity diagnostics for email deliverability. Single command (no subcommands) with an optional `--domain` flag. Extracts the domain from the configured `fromAddress` by default. Composes SES identity data (`getIdentity` for both the email address and domain) with live DNS lookups via the `DnsResolver` interface (`@/src/lib/dns.ts`). Reports five checks, each marked `[PASS]`, `[WARN]`, or `[FAIL]`:
  ...
  Sets exit code 1 if any check is FAIL. WARN checks (missing SPF/DMARC/MX) do not cause failure because these records improve deliverability but are not strictly required. DNS timeouts always produce FAIL because they indicate a network-level problem rather than a missing record. This command exists because SES generates DKIM tokens and tells users to add DNS records but never verifies they are actually present in DNS; SPF, DMARC, and MX records are critical for deliverability but SES has zero awareness of them. Depends on `newsletter.config.json` for the default domain (extracted from `fromAddress` via `extractEmail()` from `@/src/lib/email.ts`). Unlike most other commands, this one has a non-SES external dependency (`DnsResolver`) injected through the factory function rather than only using `SesService`

- **`audit` command** performs a comprehensive SES account readiness assessment by composing existing service methods and DNS lookups into a single unified report. Single command (no subcommands) with `--days` (1-60, defaults to 7) and `--domain` options. Runs all checks via `Promise.all` for parallelism and reports each as `[PASS]`, `[WARN]`, `[FAIL]`, or `[CRITICAL]`:

  | Section | Checks performed |
  |---|---|
  | Account | Sending enabled, enforcement status (HEALTHY/PROBATION/SHUTDOWN), sandbox mode |
  | Identity | From-address or domain identity exists and is verified, DKIM status |
  | DNS | SPF record includes amazonses.com, DMARC record with enforcing policy, DKIM CNAME records correct, MX records present |
  | Reputation | Bounce rate (< 2% pass, 2-5% warn, >= 5% critical), complaint rate (< 0.05% pass, 0.05-0.1% warn, >= 0.1% critical) |
  | Contacts | Contact list exists, subscribed contacts count, suppression list overlap |

  The `[CRITICAL]` label is a strict subset of failures: it is used only for enforcement SHUTDOWN, bounce rate >= 5%, and complaint rate >= 0.1%. CRITICAL checks increment the fail counter (not a separate counter). Ends with a summary line showing pass/warn/fail counts. Sets exit code 1 if any check is FAIL or CRITICAL. This command is the composition of `health`, `reputation`, `domain-check`, `setup check`, and `cleanup report` into a single exit code, designed for CI/CD integration and agentic CLI usage. Uses the same `DnsResolver` injection pattern as `domain-check`. Depends on `newsletter.config.json` for domain, from-address, contact list name, and topic name. Uses `extractEmail()` from `@/src/lib/email.ts`. Fetches identity for both the bare email and the domain, preferring the domain identity (`effectiveIdentity`) when both exist

- **`send-safe` command** combines preflight validation, suppression list filtering, and email sending into a single atomic operation. Takes a required `<html-file>` argument plus optional `--test` (comma-separated test recipients, skips preflight) and `--dry-run` (run checks without sending). The command is the safest way to send a newsletter -- it replaces the manual workflow of running `preflight` then `cleanup` then `send` separately. Runs four parallel service calls (`getAccountInfo`, `getIdentity`, `listContacts`, `listSuppressedDestinations`) for preflight checks, then filters suppressed contacts client-side using case-insensitive comparison, then sends via the extracted `sendEmails` helper with the same `p-throttle` pattern as `send`:

  | Check | PASS | WARN | FAIL |
  |---|---|---|---|
  | Account | Sending enabled | Sandbox mode or non-HEALTHY enforcement | Sending disabled |
  | Identity | From address verified | -- | Not found or not verified |
