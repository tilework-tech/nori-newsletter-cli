...
- Both `domain-check` (`@/tests/commands/domain-check.test.ts`) and `audit` (`@/tests/commands/audit.test.ts`) tests define local `createMockDnsResolver()` functions that return a `DnsResolver` implementation with configurable responses and error simulation (keyed by `type:hostname`, e.g. `cname:token-1._domainkey.example.com`). These are separate from the shared helpers because the `DnsResolver` is not part of the `SesService` interface -- it is a distinct injection point. DNS errors are simulated by attaching a `.code` property to thrown errors (e.g., `ENODATA`, `ETIMEOUT`), matching Node's `dns.promises` error conventions
