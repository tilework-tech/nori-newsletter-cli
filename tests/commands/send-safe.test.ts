...
<body><p>Hello subscribers!</p></body></html>`;
  ...
  describe("SES tracking", () => {
    ...
    it("attaches neither config set nor tags, and warns once, when not configured", async () => {
      ...
      const { exitCode, stderr } = await runCommand(ses, ["send-safe", htmlPath]);
    ...
    it("tags --test sends too", async () => {
      const { exitCode } = await runCommand(
        ses,
        ["send-safe", htmlPath, "--test", "qa@example.com"],
        trackedConfig
      );

      expect(exitCode).toBe(0);
      expect(ses.getSentEmails()[0].configurationSetName).toBe(
        "newsletter-tracking"
      );
    });
