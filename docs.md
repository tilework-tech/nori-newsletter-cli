...
- Provides both low-level SES operations (contact management, sending, suppression, templates, identities, config sets, validation, bulk import/export jobs) and high-level abstraction commands (`health` dashboard, `preflight` pre-send checks, `cleanup` contact/suppression reconciliation, `setup` infrastructure provisioning, `reputation` sending health analysis, `domain-check` DNS/deliverability diagnostics) that compose multiple service calls and/or external lookups into newsletter-specific workflows
...
- Several commands operate at the AWS account level and do not depend on `newsletter.config.json`: `suppression`, `lists`, `stats`, `templates`, `identities`, `config-sets`, `validate check`, `reputation`, and most `jobs` subcommands. Their factory functions still accept `getConfig` for consistency with the shared command factory signature. The `jobs import-contacts` subcommand defaults to the configured `contactListName` when `--list` is not provided. The abstraction commands that depend on config are `health`, `preflight`, `cleanup`, `setup`, `domain-check`, and `validate list`. Note that `reputation` is an abstraction command but operates account-level only. `domain-check` extracts the domain from the configured `fromAddress` by default but can be overridden with `--domain`
- The abstraction commands (`health`, `preflight`, `cleanup`, `setup`, `reputation`, `domain-check`) are a distinct architectural layer: they compose existing `SesService` methods (and in the case of `domain-check`, external DNS lookups) without adding new AWS API calls. They exist because SES treats suppression lists and contact lists as separate systems with no cross-referencing, has silent failure modes (suppressed addresses still count toward quota), requires multiple API calls to assemble a complete picture of newsletter health or provision the required infrastructure, provides raw metric numbers without interpreting them against enforcement thresholds, and generates DKIM tokens without verifying that the corresponding DNS records actually exist
