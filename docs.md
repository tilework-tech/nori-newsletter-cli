...
- Provides both low-level SES operations (contact management, sending, suppression, templates, identities, config sets, validation, bulk import/export jobs) and high-level abstraction commands (`health` dashboard, `preflight` pre-send checks, `cleanup` contact/suppression reconciliation, `setup` infrastructure provisioning, `reputation` sending health analysis, `domain-check` DNS/deliverability diagnostics, `audit` comprehensive account readiness assessment) that compose multiple service calls and/or external lookups into newsletter-specific workflows
