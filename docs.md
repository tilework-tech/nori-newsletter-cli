...
- Provides both low-level SES operations (contact management, sending, suppression, templates, identities, config sets, validation) and high-level abstraction commands (`health` dashboard, `preflight` pre-send checks, `cleanup` contact/suppression reconciliation, `setup` infrastructure provisioning) that compose multiple service calls into newsletter-specific workflows
